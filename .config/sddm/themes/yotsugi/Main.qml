...
// Yotsugi Layout (custom image + colors, single-sourced from theme.conf)
Rectangle {
    // Wayland Cursor Fix
    MouseArea {
        anchors.fill: parent
    ...
    id: root; width: Screen.width; height: Screen.height; color: root.bgColor
    property bool isQuickshell: typeof sddm === "undefined" || sddm.hostName === undefined
    property int sessionIndex: (typeof sessionModel !== "undefined" && sessionModel.lastIndex >= 0) ? sessionModel.lastIndex : 0
    property int userIndex: userModel.lastIndex >= 0 ? userModel.lastIndex : 0
    property real ui: 0

    // Theme colors — single source: theme.conf (read via SDDM `config`, with fallbacks for Quickshell where `config` is undefined)
    readonly property color mTeal:   (typeof config !== "undefined" && config.mTeal)           ? config.mTeal           : "#92C8C9"
    readonly property color mCream:  (typeof config !== "undefined" && config.color)            ? config.color           : "#F7EFE3"
    readonly property color mOrange: (typeof config !== "undefined" && config.mOrange)          ? config.mOrange         : "#F0C674"
    readonly property color bgColor: (typeof config !== "undefined" && config.backgroundColor)  ? config.backgroundColor : "#121118"
    readonly property string bgImage:(typeof config !== "undefined" && config.background)       ? config.background      : "bg.png"
